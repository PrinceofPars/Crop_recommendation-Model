...
xgb_model = joblib.load(r"Model/xgb_model.pkl")
le = joblib.load(r"Model/label_encoder.pkl")
scaler = joblib.load(r"Model/scaler.pkl")
hybrid_model = tf.keras.models.load_model(r"Model/hybrid_model.h5")
data = pd.read_csv(r"Model/Final.csv").fillna(method='ffill')
...
crops= {
    'Arecanut': {
        ...
        'water_requirement': {
            'min': 1500,  # mm per year
            'max': 2500,
            'description': 'Arecanut requires consistent moisture with good drainage. Needs regular irrigation in dry periods.'
        }
    },
    'Banana': {
        'temp_min': 28,
        ...
        'water_requirement': {
                'min': 1200,
                'max': 2200,
                'description': 'Bananas need large amounts of water but good drainage. Daily irrigation may be needed in dry climates.'
            }
    },
    'Black Pepper': {
        'temp_min': 25,
    ...
    'Rice': {
        ...
        'water_requirement': {
            'min': 900,
            'max': 2500,
            'description': 'Rice requires standing water for most of its growth period. Needs continuous irrigation.'
        }
    },
    'Sugarcane': {
        'temp_min': 25,
        ...
        'water_requirement': {'min': 1500, 'max': 2500, 'description': 'High water requirement throughout growth cycle'}
    },
    'Sweet potato': {
        'temp_min': 25,
        'temp_max': 30,
        ...
        'water_requirement': {
            'min': 450,  # mm per year
            'max': 650,
            'description': 'Sweet potato thrives under consistent moisture, especially during root development. Reduce watering before harvest to prevent tuber cracking.'
        }
    },
    'Arhar/Tur': {
        'temp_min': 25,
    ...
    'Cotton(lint)': {
        ...
        'water_requirement':{'min': 700, 'max': 1200, 'description': 'Needs careful water management, sensitive to waterlogging'}
    },
    'Gram': {
        'temp_min': 25,
        'temp_max': 35,
    ...
    'Groundnut': {
        ...
        'water_requirement': {'min': 500, 'max': 700, 'description': 'Moderate needs, critical during flowering and pod formation'}
    },
    'Horse-gram': {
        'temp_min': 25,
        'temp_max': 30,
    ...
    'Maize': {
        ...
        'water_requirement':{'min': 500, 'max': 800, 'description': 'Regular irrigation needed, especially during tasseling'}
    },
    'Moong(Green Gram)': {
        'temp_min': 25,
        'temp_max': 35,
    ...
    'Onion': {
        ...
        'water_requirement': {'min': 350, 'max': 550, 'description': 'Moderate needs, critical during bulb formation'}
    },
    'Potato': {
        'temp_min': 15,
        'temp_max': 25,
        ...
        'water_requirement': {'min': 500, 'max': 700, 'description': 'Consistent moisture needed, especially during tuber formation'}
    },
    'Ragi': {
        'temp_min': 20,
        'temp_max': 35,
    ...
    'Soyabean': {
        ...
        'water_requirement': {'min': 450, 'max': 700, 'description': 'Moderate needs, sensitive to drought during flowering'}
    },
    'Sunflower': {
        'temp_min': 20,
        'temp_max': 35,
    ...
    'Urad': {
        ...
        'water_requirement': {
            'min': 500,  # mm per year
            'max': 700,
            'description': 'Urad requires moderate water, with critical irrigation during flowering and pod development stages. Avoid waterlogging.'
        }
    },
    'Wheat': {
        'temp_min': 10,
        ...
        'water_requirement':{'min': 450, 'max': 650, 'description': 'Moderate water needs, most critical during flowering'}
    },
    'Jute': {
        'temp_min': 25,
        'temp_max': 38,
    ...
    'Cardamom': {
        ...
        'min': 1500,  # mm per year
        'max': 4000,
        'description': '''Cardamom requires high humidity (75-90%) and consistent soil moisture.'''
        }
    },
    'Coriander': {
        'temp_min': 17,
    ...
    'water_requirement': {'min': 600, 'max': 900, 'description': 'Regular irrigation, avoid water stress during flowering/fruiting'}
}
}
...
def fetch_real_crop_prices_from_first_api(location, crop_name, timeout=10):
    ...
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            if data.get('records'):
                return float(data['records'][0]['Modal_x0020_Price'])
        else:
    ...
    return None
def calculate_soil_fertility(user_input):
    """Calculate soil fertility based on nutrient levels and pH"""
    fertility_score = 0
    
    # Nitrogen scoring (kg/ha)
    if user_input['N'] >= 280:
        fertility_score += 30
    elif 150 <= user_input['N'] < 280:
        fertility_score += 20
    else:
        fertility_score += 10

    # Phosphorus scoring (kg/ha)
    if user_input['P'] >= 25:
        fertility_score += 25
    elif 10 <= user_input['P'] < 25:
        fertility_score += 15
    else:
        fertility_score += 5

    # Potassium scoring (kg/ha)
    if user_input['K'] >= 150:
        fertility_score += 25
    elif 50 <= user_input['K'] < 150:
        fertility_score += 15
    else:
        fertility_score += 5

    # pH scoring
    if 6.0 <= user_input['soil_pH'] <= 7.0:
        fertility_score += 20
    else:
        fertility_score += 10

    # Determine fertility rating [2][10]
    if fertility_score >= 80:
        return "Very High Fertility"
    elif 60 <= fertility_score < 80:
        return "High Fertility"
    elif 40 <= fertility_score < 60:
...
def get_stable_price(location, crop_name):
    ...
    if current_hour >= 22 or current_hour < 16:
...
def get_weather_data(location):
    # latitude, longitude = get_coordinates(location) #hhhhhhhhhhhhhhhhhhhhhhhhh

    latitude, longitude = 29.448055, 77.310280
    ...
    if response.status_code == 200:
        ...
        weather_data = {
            'humidity': current_weather['relative_humidity_2m',10],  # Humidity percentage
            'rainfall': current_weather.get('precipitation', 2),  # Rainfall in mm
...
def fetch_current_sunlight(location):
    # latitude, longitude = get_coordinates(location) #gggggggggggggggggggggg
    latitude, longitude = 29.448055, 77.310280
    if latitude is None:
...
async def fetch_weather_forecast(location, growth_days, user_input):
    # latitude, longitude = get_coordinates(location)#jjjjjjjjjjjjjjjjjjj
    latitude, longitude = 29.448055, 77.310280
    if latitude is None:
...
async def adjust_growth_duration(crop, location, user_input):
    ...
    if weather_data.get("avg_sunlight_hours") is None:
        weather_data["avg_sunlight_hours"] = fetch_current_sunlight(location)   # Default to 0 if no sunlight hours are available
...
def calculate_soil_fertility_percentage(user_input, crop_data):
    """Calculate soil fertility percentage based on nutrient levels and pH."""
    percentages = {}

    # Nitrogen (N) percentage
    percentages['N'] = min(100, (user_input['N'] / crop_data['optimal_N']) * 100)

    # Phosphorus (P) percentage
    percentages['P'] = min(100, (user_input['P'] / crop_data['optimal_P']) * 100)

    # Potassium (K) percentage
    percentages['K'] = min(100, (user_input['K'] / crop_data['optimal_K']) * 100)

    # pH percentage (within optimal range)
    if crop_data['optimal_pH_min'] <= user_input['soil_pH'] <= crop_data['optimal_pH_max']:
        percentages['pH'] = 100
    else:
        # Calculate deviation from the optimal range
        pH_diff = max(
            abs(user_input['soil_pH'] - crop_data['optimal_pH_min']),
            abs(user_input['soil_pH'] - crop_data['optimal_pH_max'])
        )
        percentages['pH'] = max(0, 100 - (pH_diff * 10))  # Penalize by 10% per unit deviation
...
def hybrid_recommendation(user_input, terrain):
    ...
    # if user_input['location']=="Assam, India":
    #     terrain="Hilly"
    if terrain == "Hilly":
        ...
        final_recs = [no_fert[0]] + [r for r in recommendations if r['needs_fertilizer']][2:4] if no_fert else recommendations[2:5]
        return final_recs

# Function to get enhanced recommendations
# Function to get enhanced recommendationshybr
async def enhanced_recommendation(user_input, terrain):
    ...
    if terrain == "Hilly":
        ...
        for idx, rec in enumerate(recommendations):
            ...
            # Ensure expected_profit is a float
            ...
            final_output.append({
                ...
                'expected_profit': expected_profit,  # Ensure this is a float
        ...
        for idx, rec in enumerate(recommendations):
            ...
            fertility_percentage = calculate_soil_fertility_percentage(user_input, crop_data)
            # Ensure expected_profit is a float
            expected_profit = float(rec['expected_profit'])
            ...
            final_output.append({
                ...
                'soil_fertility_percentage': fertility_percentage
            })
...
# Function to get weather data
def get_weather_data(location):
    # latitude, longitude = get_coordinates(location)#hhhhhhhhhhh
    latitude, longitude = 29.448055, 77.310280
    url = f"https://api.open-meteo.com/v1/forecast?latitude={latitude}&longitude={longitude}&current=temperature_2m,relative_humidity_2m,precipitation&timezone=auto"
    ...
    if response.status_code == 200:
        ...
        weather_data = {
            ...
            'rainfall': current_weather.get('precipitation', 20),  # Rainfall in mm
            'temperature': current_weather.get('temperature_2m', 25)  # Temperature in Celsius
        }
...
def populate_user_input(user_provided_input):
    ...
    # Get coordinates
    # lat, lon = get_coordinates(location)#ggggggggggggggggggggg
    
    lat, lon = 29.448055, 77.310280
    ...
    user_input = {
        ...
        'humidity':user_provided_input.get('humidity',weather_data['humidity']), # From weather data
        'rainfall': user_provided_input.get('rainfall',weather_data['rainfall']),  # From weather data
...
async def run_recommendation(user_provided_input):
    ...
    fertility = calculate_soil_fertility(user_input)
    print(f"\nSoil Fertility Rating: {fertility}")
    # Display results
    if terrain == "Hilly":
        ...
        for idx, rec in enumerate(final_recommendations, 1):
            ...
            print(f"   - Current Yield of {rec['crop1']}: {rec['current_yield1']:,.2f}")
            print(f"   - Current Yield of {rec['crop2']}: {rec['current_yield2']:,.2f}")
            print(f"   - Possible Yield of {rec['crop1']}: {rec['potential_yield1']:,.2f}")
            print(f"   - Possible Yield of {rec['crop2']}: {rec['potential_yield2']:,.2f}")
        ...
        print("\nTop 3 Profit-Maximizing Crops for Plain area -",user_input['location'],":")
        for idx, rec in enumerate(final_recommendations, 1):
            crop_data = crops[rec['crop']]
            print(f"\n{idx}. {rec['crop']}")
            print(f"  {crop_data['image']}")
            print(f"   - Growth duration: {rec['growth_duration']}")
            print(f"   - Current Yield : {rec['current_yield']}")
            print(f"   - Possible Yield : {rec['potential_yield']}")
            print(f"   - Market Value : {rec['price_per_ton']}")
            print(f"   - Expected Revenue: {rec['expected_profit']:,.2f}")
            
            # Add water requirement information
            water_req = crop_data.get('water_requirement', {})
            if water_req:
                print(f"   - Water Requirement: {water_req.get('min', 'N/A')}-{water_req.get('max', 'N/A')} mm per year")
                print(f"   - Water Notes: {water_req.get('description', 'No specific water requirements available')}")
            
            # Calculate personalized water need based on location rainfall
            rainfall = user_input.get('rainfall', 0)
            if water_req and rainfall:
                water_deficit = max(0, water_req['min'] - rainfall)
                if water_deficit > 0:
                    print(f"   - Irrigation Needed: Approximately {water_deficit:.0f} mm/year additional water required")
                else:
                    print("   - Natural rainfall should be sufficient for this crop")
            print("   - Soil Fertility Percentage:")
            for nutrient, percentage in rec.get('soil_fertility_percentage', {}).items():
                print(f"     • {nutrient}: {percentage:.2f}%")
...
async def main():
        ...
        # 'location': 'Punjab, India', #custom input but can be fetched
        # 'temperature': 21,  # Current temperature in degrees Celsius
        # 'soil_pH': 6.5,  # Average soil pH in Punjab since we currently don't have sensors integrated
        # 'humidity': 70,  # Current relative humidity percentage since we currently don't have sensors integrated
        # 'soil_moisture': 20,  # Average volumetric soil moisture percentage since we currently don't have sensors integrated
        # 'last_crop': 'wheat',  # Farmer input (user INPUT)
        # 'land_size': 5,  # Land size in hectares, (farmer input)
        # 'N': 240,  # Available nitrogen in kg/ha since we currently don't have sensors integrated
        # 'P': 11,  # Available phosphorus in kg/ha since we currently don't have sensors integrated
        # 'K': 280,  # Available potassium in kg/ha since we currently don't have sensors integrated
        # 'rainfall': 535.0 

        'location': 'Uttar Pradesh, India', #custom input but can be fetched
        'temperature': 28,  # Current temperature in degrees Celsius
        'soil_pH': 8,  # Average soil pH in Punjab since we currently don't have sensors integrated
        'humidity': 40,  # Current relative humidity percentage since we currently don't have sensors integrated
        'soil_moisture': 20,  # Average volumetric soil moisture percentage since we currently don't have sensors integrated
        'last_crop': 'sugarcane',  # Farmer input (user INPUT)
        'land_size': 5,  # Land size in hectares, (farmer input)
        'N': 170,  # Available nitrogen in kg/ha since we currently don't have sensors integrated
        'P': 30,  # Available phosphorus in kg/ha since we currently don't have sensors integrated
        'K': 280,  # Available potassium in kg/ha since we currently don't have sensors integrated
        'rainfall': 869.0 

    # 'location': 'Chennai, India',
    # 'temperature': 30,  # Current temperature in degrees Celsius
    # 'soil_pH': 7.5,  # Slightly alkaline soil
    # 'humidity': 80,  # High relative humidity percentage
    # 'soil_moisture': 25,  # Moderate soil moisture percentage
    # 'last_crop': 'rice',  # Farmer input
    # 'land_size': 3,  # Land size in hectares, farmer input
    # 'N': 180,  # Available nitrogen in kg/ha
    # 'P': 22,  # Available phosphorus in kg/ha
    # 'K': 250,  # Available potassium in kg/ha
    # 'rainfall': 1400.0  # Average annual rainfall in mm

    # 'location': 'Assam, India',
    # 'temperature': 25,  # Current temperature in degrees Celsius
    # 'soil_pH': 5.5,  # Slightly acidic soil, common in Assam
    # 'humidity': 85,  # High relative humidity percentage
    # 'soil_moisture': 40,  # High soil moisture percentage
    # 'last_crop': 'Black Pepper',  # Farmer input
    # 'land_size': 4,  # Land size in hectares, farmer input
    # 'N': 200,  # Available nitrogen in kg/ha
    # 'P': 18,  # Available phosphorus in kg/ha
    # 'K': 260,  # Available potassium in kg/ha
    # 'rainfall': 2800.0  # Average annual rainfall in mm}
